...
public class UserLastLogin extends ESTCTagLibTagSupport {
	...
	private static final Logger log = LogManager.getLogger(UserLastLogin.class);

	public int doStartTag() throws JspException {
		try {
			User theUser = (User)findAncestorWithClass(this, User.class);
			if (!theUser.commitNeeded) {
				pageContext.getOut().print(theUser.getLastLogin());
			}
		} catch (Exception e) {
	...
	public void setLastLogin(Timestamp lastLogin) throws JspException {
		try {
			User theUser = (User)findAncestorWithClass(this, User.class);
